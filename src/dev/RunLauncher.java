...
public strictfp class RunLauncher {
    ...
    static void runLauncher(RobotController rc) throws GameActionException {
        //TODO: remove the print statement when it is worked out correctly
        updateHorizontal(rc);
        updateVertical(rc);
        updateRotational(rc);
        if (!canHorizontal || !canVertical || !canRotational) {
            System.out.println ("horz " + canHorizontal + " vert " + canVertical + " rotat " + canRotational);
        }
        attackEnemies(rc);
        if (turnCount != 1)
            updateMap(rc);
